...
$facet = $facetSet->createFacetPivot('city-zip');
$facet->addFields('city,zip');
$facet->setMinCount(0);

$facet = $facetSet->createFacetPivot('zip-city');
$facet->addFields('zip,city');
...
$facetResult = $resultset->getFacetSet()->getFacet('city-zip');
echo '<h3>cat &raquo; popularity &raquo; instock</h3>';
foreach ($facetResult as $pivot) {
    displayPivotFacet($pivot);
}

$facetResult = $resultset->getFacetSet()->getFacet('zip-city');
